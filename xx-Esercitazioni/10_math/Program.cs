...
double[] arrotondati = new double[numeri1.Length]; // Array per i numeri arrotondati di lunghezza uguale a numeri
for (int i = 0; i < numeri1.Length; i++)
{
    arrotondati[i] = Math.Round(numeri[i], 2); // Arrotonda il numero alla seconda cifra decimale
    Console.WriteLine($"Numero arrotondato: {numeri1[i]}");
}
